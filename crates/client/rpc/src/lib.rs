    ...
    H: HasherT + SendSyncStatic,
{
    ...
    async fn add_declare_transaction(
        ...
        Ok(DeclareTransactionResult {
            transaction_hash: transaction.compute_hash::<H>(chain_id).into(),
    ...
    async fn add_invoke_transaction(
        ...
        Ok(InvokeTransactionResult { transaction_hash: transaction.compute_hash::<H>(chain_id).into() })
    }
    ...
    fn get_transaction_by_hash(&self, transaction_hash: FieldElement) -> RpcResult<Transaction> {
        ...
        let find_tx = block
            .transactions()
            .iter()
            .find(|tx| tx.compute_hash::<H>(chain_id).0 == transaction_hash)
