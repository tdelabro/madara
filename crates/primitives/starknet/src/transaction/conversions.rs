...
impl DeclareTransactionV0 {
    fn try_into_executable<H: HasherT>(
        ...
        let transaction_hash = self.compute_hash::<H>(chain_id);
...
impl DeclareTransaction {
    pub fn try_into_executable<H: HasherT>(
        ...
        match self {
            ...
            DeclareTransaction::V1(tx) => tx.try_into_executable::<H>(chain_id, contract_class),
            DeclareTransaction::V2(tx) => tx.try_into_executable::<H>(chain_id, contract_class),
        }
    }
}

impl InvokeTransactionV0 {
    pub fn into_executable<H: HasherT>(&self, chain_id: Felt252Wrapper) -> btx::InvokeTransaction {
        let transaction_hash = self.compute_hash::<H>(chain_id);
...
impl InvokeTransaction {
    pub fn into_executable<H: HasherT>(&self, chain_id: Felt252Wrapper) -> btx::InvokeTransaction {
        match self {
            InvokeTransaction::V0(tx) => tx.into_executable::<H>(chain_id),
            InvokeTransaction::V1(tx) => tx.into_executable::<H>(chain_id),
        }
    }
}

impl DeployAccountTransaction {
    pub fn into_executable<H: HasherT>(&self, chain_id: Felt252Wrapper) -> btx::DeployAccountTransaction {
        let account_address = self.get_account_address();
        let transaction_hash: Felt252Wrapper =
            self.compute_hash_given_contract_address::<H>(chain_id.into(), account_address).into();
