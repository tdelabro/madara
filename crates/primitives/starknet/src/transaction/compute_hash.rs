...
pub trait ComputeTransactionHash {
    fn compute_hash<H: HasherT>(&self, chain_id: Felt252Wrapper) -> Felt252Wrapper;
}
...
impl ComputeTransactionHash for InvokeTransaction {
    fn compute_hash<H: HasherT>(&self, chain_id: Felt252Wrapper) -> Felt252Wrapper {
        match self {
            InvokeTransaction::V0(tx) => tx.compute_hash::<H>(chain_id),
            InvokeTransaction::V1(tx) => tx.compute_hash::<H>(chain_id),
        }
    }
}
...
impl ComputeTransactionHash for DeclareTransaction {
    fn compute_hash<H: HasherT>(&self, chain_id: Felt252Wrapper) -> Felt252Wrapper {
        match self {
            DeclareTransaction::V0(tx) => tx.compute_hash::<H>(chain_id),
            DeclareTransaction::V1(tx) => tx.compute_hash::<H>(chain_id),
            DeclareTransaction::V2(tx) => tx.compute_hash::<H>(chain_id),
        }
    }
}
...
impl ComputeTransactionHash for DeployAccountTransaction {
    fn compute_hash<H: HasherT>(&self, chain_id: Felt252Wrapper) -> Felt252Wrapper {
        ...
        self.compute_hash_given_contract_address::<H>(chain_id, contract_address).into()
    }
}
...
impl ComputeTransactionHash for Transaction {
    fn compute_hash<H: HasherT>(&self, chain_id: Felt252Wrapper) -> Felt252Wrapper {
        match self {
            Transaction::Declare(tx) => tx.compute_hash::<H>(chain_id),
            Transaction::DeployAccount(tx) => tx.compute_hash::<H>(chain_id),
            Transaction::Invoke(tx) => tx.compute_hash::<H>(chain_id),
            Transaction::L1Handler(tx) => tx.compute_hash::<H>(chain_id),
        }
    }
}
...
impl ComputeTransactionHash for UserTransaction {
    fn compute_hash<H: HasherT>(&self, chain_id: Felt252Wrapper) -> Felt252Wrapper {
        match self {
            UserTransaction::Declare(tx, _) => tx.compute_hash::<H>(chain_id),
            UserTransaction::DeployAccount(tx) => tx.compute_hash::<H>(chain_id),
            UserTransaction::Invoke(tx) => tx.compute_hash::<H>(chain_id),
        }
    }
}
